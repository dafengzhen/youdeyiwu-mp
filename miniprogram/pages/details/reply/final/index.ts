...
import { type IPostCommentParentReply } from '@interfaces/post';
...
const finalReplyApp = getApp<IApp>();

Page({
  data: {
    pathData: null as null | IPath,
    replyData: null as null | IPostCommentParentReply,
    cacheKey: 'details_reply_final_page',
    tip: '抱歉，访问此资源遇到错误',
    showTip: false,
    hideTip: false,
    ...
    replyTarget: null as null | IPostCommentParentReply,
    replyValue: '',
    placeholder: '请输入回复',
    focus: false,
    isLoadSubmit: false,
  ...
  async onLoad(query = {}) {
    ...
    try {
      ...
      const cacheData:
        | {
            pathData: IPath;
            replyData: IPostCommentParentReply;
            id: any;
            pid: any;
          }
      ...
      let replyData: null | IPostCommentParentReply = null;
      if (cacheData === undefined) {
        const pathReq = queryPath();
        const queryAllParentReplyByReplyIdReq =
          queryAllChildReplyByParentReplyId({
            id,
          });
  ...
  async onReachBottom() {
    ...
    const response = await queryAllChildReplyByParentReplyId({
      id: this.data.id,
      query: params,
    });
  ...
  handleReplyData(item: IPostCommentParentReply) {
  ...
  async bindTapSend(e: any) {
    const item: IPostCommentParentReply = e.currentTarget.dataset.item;
    if (!item) {
  ...
  async bindFormSubmit(e: any) {
    ...
    if (!pathData.user || !!finalReplyApp.globalData._isQuickLogin) {
  ...
  bindTapUserNavigator(e: any) {
    ...
    setNavQueryStrings(finalReplyApp, { id });
  },
});
